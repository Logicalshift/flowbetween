...
fn remove_key_frame() {
    executor::block_on(async {
        ...
        test_layer_edit_undo(
            ...
            vec![
                Layer(0, RemoveKeyFrame(Duration::from_millis(1000))),
            ]
        ).await;
    });
}
...
fn remove_two_key_frames() {
    executor::block_on(async {
        ...
        test_layer_edit_undo(
            ...
            vec![
                Layer(0, RemoveKeyFrame(Duration::from_millis(3000))),
                Layer(0, RemoveKeyFrame(Duration::from_millis(1000))),
            ]
        ).await;
    });

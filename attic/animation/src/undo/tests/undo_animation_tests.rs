...
#[test]
fn undo_create_element_after_double_finish() {
    executor::block_on(async {
        use AnimationEdit::*;
        use LayerEdit::*;
        ...
        // Double finish actions don't create a new undo action (and should undo the previous action instead of undoing nothing)
        animation.edit().publish(Arc::new(vec![
            Undo(UndoEdit::FinishAction),
        ])).await;

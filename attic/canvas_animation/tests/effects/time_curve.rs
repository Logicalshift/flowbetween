...
pub fn linear_time_curve() {
    let control_points = vec![(100.0, 200.0, 300.0)];

    let p1 = TimeCurveEffect::<TestEffect>::time_for_time(&control_points, Duration::from_millis(50));
    let p1 = (p1.as_nanos() as f64) / 1_000_000.0;
    assert!((p1-50.0).abs() < 0.1);
    let p2 = TimeCurveEffect::<TestEffect>::time_for_time(&control_points, Duration::from_millis(200));
    let p2 = (p2.as_nanos() as f64) / 1_000_000.0;
    assert!((p2-200.0).abs() < 0.1);
    let p3 = TimeCurveEffect::<TestEffect>::time_for_time(&control_points, Duration::from_millis(300));
    let p3 = (p3.as_nanos() as f64) / 1_000_000.0;
    assert!((p3-300.0).abs() < 0.1);
    let p4 = TimeCurveEffect::<TestEffect>::time_for_time(&control_points, Duration::from_millis(400));
...
pub fn simple_easing_time_curve() {
    let control_points = vec![(0.0, 300.0, 300.0)];
    ...
    for p in 1..400 {
        ...
        let t1 = TimeCurveEffect::<TestEffect>::time_for_time(&control_points, Duration::from_millis(p1));
        let t2 = TimeCurveEffect::<TestEffect>::time_for_time(&control_points, Duration::from_millis(p2));
        let t3 = TimeCurveEffect::<TestEffect>::time_for_time(&control_points, Duration::from_millis(p3));

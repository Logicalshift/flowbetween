...
pub struct VirtualCanvas {
    ...
    draw_region: Arc<dyn Fn(f32, f32) -> Box<dyn Fn(&mut dyn GraphicsContext) -> ()+Send+Sync>+Send+Sync>,
...
impl VirtualCanvas {
    ...
    ///
    pub fn new<DrawRegion: Fn(f32, f32) -> Box<dyn Fn(&mut dyn GraphicsContext) -> ()+Send+Sync>+Send+Sync+'static>(canvas_resources: Arc<ResourceManager<BindingCanvas>>, draw_region: DrawRegion) -> VirtualCanvas {
        let tiles       = bind(vec![]);

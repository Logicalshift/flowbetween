...
fn retrieve_or_generate_cached_onionskin() {
    ...
    let cached_drawing  = cache.retrieve_or_generate(CacheType::OnionSkinLayer, Box::new(|| Arc::new(vec![Draw::Path(PathOp::NewPath), Draw::Fill])));
    ...
    assert!(cached_drawing == Arc::new(vec![Draw::Path(PathOp::NewPath), Draw::Fill]));

    // Should be able to retrieve instantly next time
    let cached_drawing  = cache.retrieve_or_generate(CacheType::OnionSkinLayer, Box::new(|| Arc::new(vec![Draw::Path(PathOp::NewPath), Draw::Fill])));

    assert!(match cached_drawing { CacheProcess::Cached(_) => true, _ => false });
    assert!(match cached_drawing { CacheProcess::Cached(cached_drawing) => cached_drawing == Arc::new(vec![Draw::Path(PathOp::NewPath), Draw::Fill]), _ => false });
}

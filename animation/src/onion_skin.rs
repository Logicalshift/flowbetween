            ...
            for element in elements {
                ...
                if let Some(element_path) = element.to_path(&properties) {
                    if element_path.len() > 0 {
                        let element_path = path_remove_interior_points::<_, _, Path>(&element_path, 0.01);
                        if element_path.len() == 0 {
                            println!("Remove interior points removed all points?");
                        } else {
                            match (*properties).brush.drawing_style() {
                                BrushDrawingStyle::Draw     => { onion_skin = path_add(&onion_skin, &element_path, 0.01); }
                                BrushDrawingStyle::Erase    => { onion_skin = path_sub(&onion_skin, &element_path, 0.01); }
                            }
                        }
                    }
                }

...
struct OverlayLayer {
    ...
    layers:         HashMap<LayerId, LayerId>,
...
impl CanvasRenderer {
    ...
    fn relay_drawing_for_overlay<DrawIter: Iterator<Item=Draw>>(&mut self, overlay: LayerId, gc: &mut dyn GraphicsContext, drawing: DrawIter) {
        ...
        let mut next_free_layer = move || {
            ...
            let LayerId(mut next_free_layer)    = free_layer;
            next_free_layer                     += 1;
            free_layer                          = LayerId(next_free_layer);
    ...
    ///
    fn draw_background(&mut self, gc: &mut dyn GraphicsContext, (width, height): (f64, f64)) {
    ...
    pub fn clear_annotation(&mut self, canvas: &BindingCanvas) {
        ...
        if annotated_layer.is_some() {
            canvas.draw(|gc| { gc.pop_state(); gc.restore(); gc.free_stored_buffer(); });
        }
    }

...
impl<Anim: 'static+EditableAnimation> AdjustToolState<Anim> {
    ...
    ///
    /// Returns the bezier curve corresponding to the specified start point
    ///
    pub fn curve_for_start_point(&self, control_points: &Vec<AdjustControlPoint>, control_point_index: usize) -> Option<Curve<Coord2>> {
        ...
        Some(Curve::from_points(initial_point, (cp1, cp2), end_point))
    }
    ...
    pub fn curve_at_position(&self, x: f64, y: f64) -> Option<AdjustCurvePoint> {
        ...
        for cp_index in 0..control_points.len() {
            if let Some(curve) = self.curve_for_start_point(&*control_points, cp_index) {
                ...
                if let Some((closest_t, closest_distance)) = Self::closest_t(&curve, &Coord2(x, y)) {
                    ...
                    if let Some(closest_point) = &closest {
                        if closest_point.distance > closest_distance {
                            ...
                            closest = Some(AdjustCurvePoint { start_point: (&control_points[cp_index]).into(), t: closest_t, distance: closest_distance });
                        }
                    } else {
    ...
    pub fn adjusted_control_points_for_curve_drag(&self, adjust_curve: &AdjustCurvePoint, dx: f64, dy: f64) -> Vec<(f32, f32)> {
        ...
        let curve           = if let Some(curve) = curve { curve } else { return vec![]; };

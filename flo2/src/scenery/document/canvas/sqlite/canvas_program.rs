    ...
    while let Some(msg) = input.next().await {
        ...
        match msg {
            Edit(edits) => {
                ...
                for edit in edits {
                    match edit {
                        ...
                        SetShapeTime(shape_id, when)                        => { changed_shapes.insert(shape_id); canvas.set_shape_time(shape_id, when).ok(); }
                        ...
                        SetShapeParent(shape_id, parent) => {
                            ...
                            match &parent {
                                CanvasShapeParent::Layer(layer_id, _)  => { changed_layers.insert(*layer_id); }
            ...
            Query(query) => {
                ...
                match query {
                    WholeDocument(target)                                        => { canvas.send_vec_query_response(target, &context, |canvas, response| canvas.query_document_whole(response)).await.ok(); },

...
///
async fn event_relay_program(drawing_events: impl Unpin + Send + Sink<DocumentRequest>, window_properties: WindowProperties, document_program_id: SubProgramId, drawing_window_program_id: SubProgramId, input: InputStream<DrawEvent>, context: SceneContext) {
    let mut input           = input.ready_chunks(100);
    let mut drawing_events  = drawing_events;
    let mut scale           = 1.0;
    let mut size            = (0.0, 0.0);
    let mut drawing_reqs    = context.send(drawing_window_program_id).unwrap();

    while let Some(event_chunk) = input.next().await {
        let mut update_viewport = false;
        ...
        if update_viewport {
            // Send a request to update the viewport bounds (note: this doesn't update when the binding changes, only when the size changes)
            let new_viewport_bounds = window_properties.viewport_bounds().get();
            drawing_reqs.send(DrawingWindowRequest::SetViewportBounds(new_viewport_bounds)).await.ok();
        }
    }
}
...
async fn create_empty_document(scene: Arc<Scene>, document_program_id: SubProgramId, context: &SceneContext) {
    ...
    let window_properties   = window_properties.with_viewport_bounds(computed(move || {
        ...
        let actual_scale = actual_scale.get();

        ViewportBounds::FitExact((0.0, 0.0), (actual_size.0/actual_scale, actual_size.1/actual_scale))
    }));
    ...
    scene.add_subprogram(event_relay_program_id, move |input, context| event_relay_program(drawing_events, drawing_properties, document_program_id, drawing_window_program_id, input, context), 20);
